...
class Model
{
	...
	public function max($fields)
	{
		...
		$sql = "SELECT MAX('$fields') as m FROM %TABLE%";
		$sql = str_replace('%TABLE%',$this->parseTable(),$sql);
		//echo $sql;
		$data = $this->query($sql);
	...
	public function min($fields)
	{
		...
		$sql = "SELECT MIN('$fields') as m FROM %TABLE%";
		$sql = str_replace('%TABLE%',$this->parseTable(),$sql);
		//echo $sql;
		$data = $this->query($sql);
	...
	public function sum($fields)
	{
		...
		$sql = "SELECT SUM('$fields') as m FROM %TABLE%";
		$sql = str_replace('%TABLE%',$this->parseTable(),$sql);
		//echo $sql;
		$data = $this->query($sql);
	...
	public function count($fields)
	{
		...
		$sql = "SELECT COUNT('$fields') as m FROM %TABLE% %WHERE%";
		$sql = str_replace(
			array('%TABLE%','%WHERE%'),
			array(
					$this->parseTable(),
	...
	public function avg($fields)
	{
		...
		$sql = "SELECT AVG('$fields') as m FROM %TABLE%";
		$sql = str_replace('%TABLE%',$this->parseTable(),$sql);
		//echo $sql;
		$data = $this->query($sql);
	...
	protected function exec($sql , $bool = null)
	{
		if ($bool) {
			...
			//var_dump($result);
			if ($result) {
				//$d =mysqli_affected_rows($this->link);
				//echo $d;
